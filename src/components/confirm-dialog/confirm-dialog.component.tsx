      ...
      <div className={clsx(classes.root, className)}>
        <Dialog maxWidth='sm' open={isVisible} onClose={onFalse}>
            <DialogTitle className={clsx(classes.dialogTitle, dialogClass)}>
              {icon}<Typography className={classes.dialogTitleText}>{title}</Typography>
            </DialogTitle>
            <DialogContent className={classes.dialog}>
              <Typography className={classes.dialogContentText}>{message}</Typography>
            </DialogContent>
            <DialogActions>
              <Button onClick={onFalse} color='secondary'>{buttonFalseText}</Button>
              <Button onClick={onTrue} color='primary' autoFocus>{buttonTrueText}</Button>
            </DialogActions>
          </Dialog>
        </div>
      );
